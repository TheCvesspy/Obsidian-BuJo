...
- **Today gets a next-3-days look-ahead.** The Today view showed only what was live *right now* (overdue + due today), so the shape of the rest of the week was invisible without switching to Upcoming — and a heavy Wednesday only announced itself on Wednesday. A muted **"Next N days"** section now sits below today's list, bucketed by due day (**Tomorrow**, then `Wednesday, Aug 19`), rendered with the same collapsible task rows so anything in it can be completed, rescheduled or snoozed in place. It is deliberately demoted (dimmed until hovered, muted uppercase header, separated by a rule) — context, not the to-do list. The header count now reads `(4 to do · 7 coming up)`, the search box filters the look-ahead too, and an empty today no longer hides the preview: "All clear for today 🎉" and the next days both show. Window is the new **`todayLookaheadDays`** setting (*Today look-ahead (days)*, default **3**, clamped 0–14; **0** hides the section entirely). Days with nothing due are skipped rather than shown blank. Snoozed tasks stay out — a task waking in two days isn't due in two days, so it keeps living under Upcoming's "Waking soon". ([src/ui/components/TodayView.ts](src/ui/components/TodayView.ts), [src/types.ts](src/types.ts), [src/settings.ts](src/settings.ts), [styles.css](styles.css))
...
- **1:1 prep agendas now assemble the member's full topic picture.** `buildOneOnOneAgenda` (used by "Start 1:1" from the palette picker and the Team Overview button) previously listed only driving / manually-blocked / waiting-on items. It now also includes: **derived-blocked** topics (dependency blocks, not just the manual flag — new `MemberRollup.blockedTopics`), **at-risk** topics with reasons, topics **sitting in In Progress** past the aging threshold, topics **done since the last 1:1**, and — new — the **dated `## Notes` log entries written since the last 1:1** across the member's topics (read from topic bodies, newest first, capped at 12). The "since" boundary is the member's previous 1:1 session date (fallback: last 14 days). The agenda lands in the session page's `## Prep (auto)` section as before. ([src/services/teamRollupService.ts](src/services/teamRollupService.ts), [src/main.ts](src/main.ts), [src/ui/components/TeamOverviewView.ts](src/ui/components/TeamOverviewView.ts), [src/types.ts](src/types.ts))

- **At-risk detection — the early-warning band before work is overdue.** New `deriveTopicRisk` (`topicStatus.ts`, finally using the reserved at-risk concept): a non-done, non-snoozed topic with a due date is **at risk** when it's overdue, or due within 7 days while (a) not started (Backlog/To Do), (b) under 50% task progress, or (c) blocked. Surfaces: an amber **AT RISK** badge on cards (reasons in the tooltip), a ⚠ marker in the List view, a **"⚠ At risk" section in the Morning Review** (owner + reasons + Open button, sorted soonest-due first), and an **At risk** section in the 1:1 prep agenda. Snoozed topics never nag — deliberate deferral suppresses risk. ([src/services/topicStatus.ts](src/services/topicStatus.ts), [src/ui/components/TopicCard.ts](src/ui/components/TopicCard.ts), [src/ui/components/TopicsOverviewView.ts](src/ui/components/TopicsOverviewView.ts), [src/ui/MorningReviewModal.ts](src/ui/MorningReviewModal.ts), [styles.css](styles.css))

- **Capacity visibility at the point of assignment and planning.** New shared helper `buildTopicLoadIndex` (`utils/capacity.ts`): committed topic load per member (To Do + In Progress, snoozed excluded) vs the effective target (`targetConcurrentItems` fallback `defaultTargetConcurrentItems`, scaled by `availabilityPercent` — the same math as the team roll-up, so numbers agree everywhere). Surfaces: **owner dropdowns** (card quick-edit + topic modal) show `Name · 3/5` with ⚠ when over; picking an owner who ends up over target raises a non-blocking **heads-up Notice**; **Roadmap assignee lanes** get a load chip (`3/5`) that turns red when over capacity. ([src/utils/capacity.ts](src/utils/capacity.ts), [src/ui/components/TopicsOverviewView.ts](src/ui/components/TopicsOverviewView.ts), [src/ui/SprintTopicModal.ts](src/ui/SprintTopicModal.ts), [styles.css](styles.css))
