...
| **Topics** | `FridayViewMode.Topics` | Topic browser with **👤 Mine / 👥 Team ownership tabs** (when identity + assignment are in use). Scope chips: All / Backlog / Done, plus an assignee filter. Four sub-modes: **Board** (Kanban — Backlog \| To Do \| In Progress \| Done — with Unassigned + Blocked strips and a Snoozed shelf), **List** (flat table), **Roadmap** (Gantt-style timeline), **Impact/Effort** (Quick Wins / Big Bets / Fill-ins / Time Sinks + Inbox). See §15. |
...
- **Ownership tabs (all sub-modes).** When `settings.jiraEmail` is set *and* at least one topic has an assignee, the view grows a **👤 Mine / 👥 Team** tab bar (with live counts) that scopes every sub-mode: Mine = my + unassigned topics, Team = topics assigned to others. The choice persists at module level. Without an identity or without any assignees, no tabs render — solo vaults keep the classic single view. On the Team tab, assignees render prominently: a colored initials avatar (deterministic hue from the email hash, `assigneeColor`/`assigneeInitials` in `TopicCard.ts`) + bold name right under the card title, and avatar + bold name in the List's assignee column.
- **Board** — status-driven Kanban with drag-and-drop, structured in layers:
  - **Unassigned strip (Mine tab).** Unassigned topics leave the columns and gather in a **📥 Unassigned strip** (drop a card there to clear its owner) — unowned work is the lead's to hand out.
  - **Blocked strip.** Blocked topics leave the status columns entirely and gather in a **🛑 Blocked strip**. Drop a card on the strip to set the manual flag; drag a blocked card into any status column and the manual flag clears ("work resumes here") — a Notice explains when a dependency/JIRA signal keeps it in the strip anyway.
  - **Snoozed shelf.** Snoozed topics of the active tab park on a collapsed **💤 Snoozed shelf** at the bottom (module-level expand state), sorted by wake date.
  - **WIP limits** (`settings.wipLimits`) are board-wide policy: the column pill shows the cross-tab total (`total / limit`) computed from the pre-tab-cut filtered set (`boardTotalsSource`), so the limit means the same thing whichever tab is open. Drops that breach warn but never block (Kanban convention).
...
Header chips scope every sub-mode: **All** (live + recently-done — topics done longer than `hideDoneAfterDays` are hidden), **Backlog** (`status === 'backlog'`), **Done** (`status === 'done'`, the escape hatch that shows everything completed). An **assignee dropdown** (shown when team members exist) offers per-member lenses; when the Mine/Team tabs are inactive it also carries the 👤 Mine / 📨 Assigned out / ∅ Unassigned options (the tabs replace those — the dropdown is hidden entirely on the Mine tab, and stale ownership values are sanitized back to "all" when the split activates). Ownership tab, scope, assignee filter, roadmap zoom/grouping/centre, and the Snoozed shelf's expand state are backed by **module-level variables** — the view object is rebuilt on every data refresh, so instance fields would silently reset the user's choices after any topic write.
